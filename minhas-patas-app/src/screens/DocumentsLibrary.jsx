    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)',
      ...
      <div style={{ width:'100%', background:T.bg, borderRadius:'24px 24px 0 0',
        ...
        <div style={{ display:'flex', gap:12, marginTop:20 }}>
          ...
          <button onClick={() => title.trim() && onSave({ cat, title, date, notes,
...
export default function DocumentsLibrary() {
  const { back } = useNav();
  const { activePet, documents, addDocument } = usePet();
  ...
  const catFilter = (d) => {
    ...
    const map = { Receitas:'Receita', Exames:'Exame', Vacinas:'Vacina', Cirurgias:'Cirurgia', Outros:'Outro' };
    return d.cat === map[cat];
  };
    ...
    <div style={{ height:'100%', display:'flex', flexDirection:'column', background:T.bg, position:'relative' }}>
      ...
      <div style={{ padding:'12px 20px 0' }}>
        <div style={{ background:T.surface, borderRadius:14, padding:'11px 16px',
          ...
          {search && (
            <div onClick={() => setSearch('')} style={{ fontSize:18, color:T.inkSoft, cursor:'pointer', lineHeight:1 }}>×</div>
          )}
        </div>
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'16px 20px 96px' }}>
        {documents.length === 0 ? (
          <div style={{ textAlign:'center', padding:'60px 20px' }}>
            ...
            <div style={{ fontWeight:800, fontSize:18, color:T.ink, marginTop:12, fontFamily:FONT_BODY }}>
              Nenhum documento ainda
            </div>
            <div style={{ fontSize:14, color:T.inkSoft, marginTop:8, maxWidth:240, margin:'8px auto 0', lineHeight:1.5 }}>
              Adicione receitas, exames, vacinas e outros documentos do seu pet.
            </div>
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign:'center', padding:'60px 20px' }}>
            <div style={{ fontSize:48 }}>🔍</div>
            <div style={{ fontSize:16, fontWeight:700, color:T.ink, marginTop:12 }}>
              Nenhum resultado
            </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            {filtered.map((d, i) => (
              <div key={d.id || i} onClick={() => setDetail(d)}
                style={{ background:T.surface, borderRadius:16, overflow:'hidden',
                  boxShadow:'0 2px 8px rgba(20,20,30,0.05)', cursor:'pointer' }}>
                <div style={{ background:d.tint || T.tintCream, padding:'20px 0', textAlign:'center', fontSize:36,
                  position:'relative' }}>
                  {d.e || '📄'}
      ...
      {detail && <DetailModal doc={detail} onClose={() => setDetail(null)} />}
    </div>
  );
}
